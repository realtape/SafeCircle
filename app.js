...
async function sendEmailAlert() {
  ...
  try {
    ...
    flashButtonLabel(elements.shareEmailButton, error.message || "Send failed");
  } finally {
...
function renderEmailRecipients() {
  ...
  const emailContacts = state.contacts.filter((contact) => isEmailAddress(contact.method.trim()));
  ...
  for (const contact of emailContacts) {
    ...
    checkbox.value = contact.method.trim();
    checkbox.checked = true;

    const text = document.createElement("span");
    text.textContent = `${contact.name} (${contact.method.trim()})`;
